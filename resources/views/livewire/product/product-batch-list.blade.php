                    ...
                    <div class="relative pl-6 space-y-8 before:absolute before:inset-y-0 before:left-[7px] before:w-[2px] before:bg-border/50">
                        ...
                        <div class="relative">
                            ...
                            <div class="space-y-2">
                                ...
                                {{-- Price & Date Changes log --}}
                                @if($log->changes)
                                <div class="p-3 rounded-lg bg-primary/5 border border-primary/10 text-[11px] space-y-2">
                                    @if(isset($log->changes['old']['harga_beli']))
                                    <div class="flex justify-between items-center">
                                        <span class="text-muted-foreground">Harga Beli</span>
                                        <div class="flex items-center gap-2">
                                            <span class="text-muted-foreground line-through italic">Rp {{ number_format($log->changes['old']['harga_beli'], 0, ',', '.') }}</span>
                                            <x-lucide-arrow-right class="h-3 w-3 text-primary" />
                                            <span class="font-bold text-primary">Rp {{ number_format($log->changes['new']['harga_beli'], 0, ',', '.') }}</span>
                                        </div>
                                    </div>
                                    @endif
                                    @if(isset($log->changes['old']['harga_jual']))
                                    <div class="flex justify-between items-center">
                                        <span class="text-muted-foreground">Harga Jual</span>
                                        <div class="flex items-center gap-2">
                                            <span class="text-muted-foreground line-through italic">Rp {{ number_format($log->changes['old']['harga_jual'], 0, ',', '.') }}</span>
                                            <x-lucide-arrow-right class="h-3 w-3 text-primary" />
                                            <span class="font-bold text-primary">Rp {{ number_format($log->changes['new']['harga_jual'], 0, ',', '.') }}</span>
                                        </div>
                                    </div>
                                    @endif
                                    @if(isset($log->changes['old']['tanggal_masuk']))
                                    <div class="flex justify-between items-center">
                                        <span class="text-muted-foreground">Tgl Masuk</span>
                                        <div class="flex items-center gap-2">
                                            <span class="text-muted-foreground line-through italic">{{ date('d/m/Y', strtotime($log->changes['old']['tanggal_masuk'])) }}</span>
                                            <x-lucide-arrow-right class="h-3 w-3 text-primary" />
                                            <span class="font-bold text-primary">{{ date('d/m/Y', strtotime($log->changes['new']['tanggal_masuk'])) }}</span>
                                        </div>
                                    </div>
                                    @endif
                                </div>
